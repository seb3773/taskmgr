...
void TQtMvcTreeView::keyPressEvent(TQKeyEvent* e)
{
...
void TQtMvcTreeView::resizeEvent(TQResizeEvent* e)
{
...
void TQtMvcTreeView::updateSearch()
{
    if (m_searchText.isEmpty()) {
        hideSearch();
        return;
    ...
    if (m_model) {
        ...
        for (int r = 0; r < numRows(); ++r) {
            int id = nodeId(r);
            if (id >= 0) {
                TQString name = m_model->data(TQtModelIndex(id, 0)).toString();
...
void TQtMvcTreeView::hideSearch()
{
    m_searchText = "";
    if (m_searchWidget) {
        m_searchWidget->hide();
