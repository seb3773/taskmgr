...
# Copy binary
cp -a "$BIN_PATH" "$APPDIR/usr/bin/taskmgr"

# Strip staged binary
if command -v sstrip >/dev/null 2>&1; then
	echo "info: stripping staged binary with sstrip"
	sstrip "$APPDIR/usr/bin/taskmgr" >/dev/null 2>&1 || true
else
	echo "info: using strip --strip-all"
	strip --strip-all "$APPDIR/usr/bin/taskmgr" >/dev/null 2>&1 || true
fi
...
if test -f "$ICON_SRC"; then
	cp -a "$ICON_SRC" "$APPDIR/taskmgr.png"
	cp -a "$ICON_SRC" "$APPDIR/usr/share/icons/hicolor/64x64/apps/taskmgr.png"
else
...
EOF
chmod 0644 "$APPDIR/taskmgr.desktop"
cp -a "$APPDIR/taskmgr.desktop" "$APPDIR/usr/share/applications/taskmgr.desktop"
...
OUT_APPIMAGE="$SRC_ROOT/taskmgr-x86_64.AppImage"
rm -f -- "$OUT_APPIMAGE"
